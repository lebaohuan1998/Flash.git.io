...
public class UserProfileInfoDAO {
	...
	public User getInfo(int id) {
		try {
			...
			while (rs.next()) {
				...
				String bod= rs.getString("dob");
				String address= rs.getString("address");
				String phone= rs.getString("phone");
				String gender= rs.getString("gender");
				String avata= rs.getString("avatar");
				nu= new User(name, pwd, phone, email, address, bod,avata);
				nu.setGender(gender);
			}
			return nu;
	...
	public boolean updateInfo(User nu, int id) {
		try {
			connection = ConnectionClass.createConnect().getConnection();
	...
	public boolean updateActive(String status, String email) {
		try {
			connection = ConnectionClass.createConnect().getConnection();
			preparedStmt = connection.prepareStatement(NormalUserQuery.UPDATEACTIVEMAIL);
			preparedStmt.setString(1, status);
	...
	public boolean updatePassByEmail( String pass, String email) {
		try {
			connection = ConnectionClass.createConnect().getConnection();
			preparedStmt = connection.prepareStatement(NormalUserQuery.UPDATE_PASSWORDEMAIL );
			preparedStmt.setString(1, pass);
			preparedStmt.setString(2, email);
			preparedStmt.execute();
			return true;
